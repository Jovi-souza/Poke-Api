  ...
  return (
    <Dialog.Root>
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="flex flex-col justify-center items-center bg-gray-500 rounded-md px-4 py-8 fixed top-[50%] left-[50%] -translate-y-[50%] -translate-x-[50%] w-[70%] max-w-xs">
          <Dialog.Title className="">
            <Image src={pokemon.url} alt="" width={150} height={150} />
          </Dialog.Title>
          <Dialog.Description className="grid grid-cols-2 gap-4 grid justify-center items-center w-4/5 bg-gray-100 rounded-md px-4 py-2">
            ...
            {pokemon.types.map(
              (item: { slot: number; type: { name: string } }) => {
                return (
                  <p
                    key={item.slot}
                    className="bg-purple-700 text-gray-100 px-2 w-max rounded"
                  >
                    {item.type.name}
                  </p>
                )
