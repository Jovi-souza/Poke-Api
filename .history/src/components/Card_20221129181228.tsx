    ...
    <Dialog.Root>
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="flex flex-col justify-center items-center bg-gray-500 rounded-md fixed top-[50%] left-[50%] -translate-y-[50%] -translate-x-[50%] w-[70%]">
          ...
          <Dialog.Description className="flex flex-wrap gap-4 justify-center items-center w-1/3 bg-red-500 ">
            ...
            {pokemon.types.map(
              (item: { slot: number; type: { name: string } }) => {
                return (
                  <p key={item.slot} className="bg-gray-200 text-gray-700 px-2 rounded">
                    {item.type.name}
                  </p>
                )
              },
